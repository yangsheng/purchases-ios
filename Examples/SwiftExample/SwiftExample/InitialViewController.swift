...
class InitialViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if let userIdentifier = UserDefaults.standard.string(forKey: "signedInUserID") {
            if #available(iOS 13.0, *) {
                ...
                appleIDProvider.getCredentialState(forUserID: userIdentifier) { (credentialState, error) in
                    DispatchQueue.main.async {
                        ...
                        case .authorized:
                            Purchases.shared.identify(userIdentifier) { (info, error) in
                                self.checkForEntitlement(purchaserInfo: info, error: error)
                            }
                            break
                        case .revoked:
                            self.showSignIn()
    ...
    private func checkForEntitlement(purchaserInfo: Purchases.PurchaserInfo?, error: Error?) {
        if let e = error {
